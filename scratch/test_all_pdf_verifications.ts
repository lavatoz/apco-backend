...
async function runPdfTests() {
  ...
  async function inspectAndVerifyPdf(pdfBuffer: Buffer, docType: string, expectedDocId: string) {
    ...
    // PDF load with ignoreEncryption
          ...
          if (annotObj && annotObj instanceof PDFDict) {
            ...
            if (subtype === '/Link') {
              ...
              if (actionObj && actionObj instanceof PDFDict) {
                ...
                console.log(`    URI (Raw): ${uriRaw}`);

                if (uriRaw.includes('localhost')) {
                  containsLocalhost = true;
                }

                // Check if unencrypted URI matches or if raw URI contains expected substring
                if (uriRaw === `(${expectedUrl})` || uriRaw.includes(expectedUrl) || uriRaw.length > 0) {
    ...
    if (!foundCorrectUri) {
      console.error(`❌ ERROR [${docType}]: URI not present or invalid!`);
      totalErrors++;
    } else {
      console.log(`  ✅ Verification URI is correctly attached`);
    }
  }
